...
func (s *transactionsSuite) TestUpdateAddressHistoryDuplicateTxs() {
	cases := []struct {
		...
		// after UpdateAddressHistory returns. Unused when wantErr.
		wantHeight int
		wantErr    bool
	}{
		{
		...
		},
		{
			...
			// DBUpdate, which UpdateAddressHistory propagates to the
			// caller instead of panicking.
			name:    "two confirmed duplicates error",
			heights: []int{10, 11},
			wantErr: true,
		},
		{
			...
			// arm above.
			name:    "two identical mempool entries error",
			heights: []int{0, 0},
			wantErr: true,
		},
		{
			...
			// parents unconfirmed").
			name:    "mempool then unconfirmed-parent errors",
			heights: []int{0, -1},
			wantErr: true,
		},
	}
	...
	for _, tc := range cases {
		s.Run(tc.name, func() {
			...
			if tc.wantErr {
				// In the error case, DBUpdate returns the
				...
				s.Require().Error(s.transactions.UpdateAddressHistory(
